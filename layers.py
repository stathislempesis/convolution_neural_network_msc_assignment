...
class MLP(object):
    ...
    def fpropAutoencoder(self, x):
        ...
        for i in xrange(0, len(self.layers)):
            self.activations[i+1] = self.layers[i].fprop(self.activations[i])
        return self.activations[len(self.activations)-3]
...
class Sigmoid(Linear):
    ...
    def __init__(self, idim, odim,
        ...
        super(Sigmoid, self).__init__(idim=idim, odim=odim, rng=rng, irange=irange)
...
class Softmax(Linear):
    ...
    def __init__(self, idim, odim,
        ...
        super(Softmax, self).__init__(idim=idim, odim=odim, rng=rng, irange=irange)
        
    def fprop(self, inputs):
        
        return softmax(super(Softmax, self).fprop(inputs))
...
def softmax(x):
    ...
    # so that we can normalize a vector or per matrix's column
    if x.ndim == 1:
        return e / numpy.sum(e)
    else :
        return  e / numpy.sum(e, axis=1)[:, numpy.newaxis]
